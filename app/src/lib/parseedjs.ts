...
const FileBlock = (block: any) => {
    ...
    <div class= "cdx-attaches cdx-attaches--with-file" >
            <div class="cdx-attaches__file-icon" >
                ...
                <div class="cdx-attaches__title" data-placeholder=${data.file.name} data-empty="true">${data.file.name}</div>
            </div>
...
const edjsParser = edjsHTML({ Attaches: FileBlock });


export const getHtmlFromEdjs = (clean_data: string, json: boolean) => {
    const editorjs_data = json ? JSON.parse(clean_data) : clean_data;
    // const isValid = edjsParser.validate(editorjs_data); 
    return edjsParser.parse(editorjs_data);
}
